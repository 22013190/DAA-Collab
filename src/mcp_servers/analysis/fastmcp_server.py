...
def perform_advanced_eda_on_csv(
    file_path: Annotated[str, Field(description="Path to the CSV file to analyze")],
    # NOTE: These are accepted for backwards-compatibility with orchestrators that
    # use a common (target_column/value_column/group_column) calling convention.
    # They do not change the core EDA behavior.
    target_column: Annotated[
        str,
        Field(
            description="(Optional) Compatibility alias: column of interest (categorical or numeric)."
        ),
    ] = None,
    value_column: Annotated[
        str,
        Field(
            description="(Optional) Compatibility alias: numeric value column for aggregation."
        ),
    ] = None,
    group_column: Annotated[
        str,
        Field(
            description="(Optional) Compatibility alias: grouping column for comparisons."
        ),
    ] = None,
...
def analyze_csv_patterns(
    ...
    top_n: Annotated[
        int,
        Field(
            description="Number of top categories to report when aggregating categorical target_column (default 5)"
        ),
    ] = 5,
    ...
    - For categorical + value_column: reports top-N categories by average value
    - With group_column: compares those top categories across groups
    ...
    try:
        ...
        if target_column and target_column in df.columns:
            ...
            # Extended mode: categorical target aggregated over a numeric value column.
            if (
                value_column
                and value_column in df.columns
                and not pd.api.types.is_numeric_dtype(col_data)
                and pd.api.types.is_numeric_dtype(df[value_column])
            ):
                safe_top_n = max(1, int(top_n or 5))

                grouped = (
                    df.groupby(target_column, dropna=False)[value_column]
                    .mean()
                    .sort_values(ascending=False)
                )
                top_series = grouped.head(safe_top_n)
                top_categories = [
                    (str(idx) if idx is not None else "<NA>") for idx in top_series.index
                ]

                patterns.append(
                    f"\n### Top {safe_top_n} {target_column} by mean {value_column}:")
                for rank, (cat, mean_val) in enumerate(top_series.items(), start=1):
                    cat_str = str(cat) if cat is not None else "<NA>"
                    patterns.append(f"- {rank}. {cat_str}: mean {value_column} = {mean_val:.4f}")

                if group_column and group_column in df.columns:
                    sub = df[df[target_column].isin(top_series.index)].copy()

                    # Compare within each group for the same top categories.
                    means = (
                        sub.groupby([group_column, target_column], dropna=False)[value_column]
                        .mean()
                        .unstack(target_column)
                    )
                    counts = (
                        sub.groupby([group_column, target_column], dropna=False)
                        .size()
                        .unstack(target_column)
                    )

                    patterns.append(
                        f"\n### Mean {value_column} for top causes by {group_column}:")
                    patterns.append(means.round(4).to_markdown())
                    patterns.append(
                        f"\n### Counts for top causes by {group_column}:")
                    patterns.append(counts.fillna(0).astype(int).to_markdown())
